...
const Card3d = ({
	...
	return (
		<StyledWrapper>
			<div className="relative inline-flex overflow-hidden rounded-[20px] p-[1px]">
        <span
          className="absolute inset-[-1000%] animate-[spin_4s_linear_infinite] bg-[conic-gradient(from_90deg_at_50%_50%,transparent_0%,#f3f4f6_10%,#f3f4f6_20%,transparent_40%)]"
          style={{ opacity: 0.7 }}
        />
				<div className="card relative inline-flex rounded-[20px] bg-slate-950 backdrop-blur-3xl">
            ...
            <h2>
              <span className="inline-bloc title-gradient">
                {title}
              </span>
            </h2>
						<p className="subtitle font-medium">{subtitle}</p>
						<p className="description font-md">{description}</p>
            <div className={`font-xs ${badgeWrapperClass ?? ""}`} onClick={onPlay}>
              <PlayButton />
            </div>
					</div>
				</div>
...
const StyledWrapper = styled.div`
  ...
  .heading h2 .title-gradient {
    font-family: 'Inter', system-ui, -apple-system, 'Segoe UI', Roboto,
      'Helvetica Neue', Arial, sans-serif;
    font-weight: 700;
    background: linear-gradient(90deg, #ffffff 0%, #e5e7eb 100%); /* white -> gray-200 */
  ...
  .heading::after {
    ...
    outline: none;
    border-radius: 50%;
    background: #9ca3af; /* gray-400 - slightly darker */
    /* gray-400 based glows */
    box-shadow: 0px 0px 140px rgba(156, 163, 175, 0.65),
      0px 0px 90px rgba(107, 114, 128, 0.5), inset #9ca3af 0px 0px 40px -12px;
    transition: 0.4s ease-in-out;
    z-index: -1;
  }
  .card:hover .heading::after {
    box-shadow: 0px 0px 520px rgba(156, 163, 175, 0.95),
      0px 0px 320px rgba(107, 104, 128, 0.85), inset #9ca3af 0px 0px 140px -12px;
  }
