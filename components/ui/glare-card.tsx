...
export const GlareCard = ({
	...
	const backgroundStyle = {
		// Use grayscale / dark foil textures for a black & gray appearance
		"--step": "6%",
		"--foil-svg": `url("data:image/svg+xml,%3Csvg width='26' height='26' viewBox='0 0 26 26' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M2.99994 3.419C2.99994 3.419 21.6142 7.43646 22.7921 12.153C23.97 16.8695 3.41838 23.0306 3.41838 23.0306' stroke='%23222' stroke-width='5' stroke-miterlimit='3.86874' stroke-linecap='round' style='mix-blend-mode:darken'/%3E%3C/svg%3E")`,
		"--pattern": "var(--foil-svg) center/100% no-repeat",
		"--rainbow":
			"repeating-linear-gradient( 0deg, rgba(255,255,255,0.02) calc(var(--step) * 1), rgba(255,255,255,0.015) calc(var(--step) * 2), rgba(0,0,0,0.04) calc(var(--step) * 3) ) 0% var(--bg-y)/200% 700% no-repeat",
		"--diagonal":
			"repeating-linear-gradient( 128deg, #050505 0%, #0b0b0b 3.8%, #0b0b0b 4.5%, #0b0b0b 5.2%, #050505 10%, #050505 12% ) var(--bg-x) var(--bg-y)/300% no-repeat",
		"--shade":
			"radial-gradient( farthest-corner circle at var(--m-x) var(--m-y), rgba(255,255,255,0.06) 6%, rgba(255,255,255,0.03) 14%, rgba(255,255,255,0) 60% ) var(--bg-x) var(--bg-y)/300% no-repeat",
		...
		>
			<div className="grid h-full origin-center overflow-hidden rounded-[var(--radius)] border border-slate-800 transition-transform delay-[var(--delay)] duration-[var(--duration)] ease-[var(--easing)] will-change-transform [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] hover:filter-none hover:[--duration:200ms] hover:[--easing:linear] hover:[--opacity:0.6]">
				<div className="grid h-full w-full mix-blend-soft-light [clip-path:inset(0_0_0_0_round_var(--radius))] [grid-area:1/1]">
					<div
						className={cn(
							"h-full w-full bg-gradient-to-b from-slate-900 to-slate-800",
							className,
						)}
					>
						{children}
